...
| 桌面壳 | Tauri 2.x (Rust) | 体积小（<10MB），比 Electron 轻量 20 倍 |
...
| **0.1.0** | Tauri 桌面 MVP 发布 | Tauri 2.x + TDD + M6/M8 | 桌面安装包 + 测试覆盖率报告 + 门禁审计报告 | M6 Reliability Gate 全通 + M8 Quality Gate 全通 + 核心测试覆盖 ≥ 80% |
...
```batch
@echo off
:: dev.bat — DevPulse 开发环境一键启动
echo Starting DevPulse Development Environment...

:: 启动后端 FastAPI（端口 8000）
start "DevPulse Backend" cmd /k "cd /d E:\Github Project\DevPulse\api-server && uvicorn main:app --reload --port 8000"

:: 等待后端启动
timeout /t 3 /nobreak >nul

:: 启动前端 dev server（端口 5173）
start "DevPulse Frontend" cmd /k "cd /d E:\Github Project\DevPulse\desktop && npm run dev"

:: 打开浏览器
start "" "http://localhost:5173"

echo DevPulse is running: Backend on :8000, Frontend on :5173
```
...
```batch
@echo off
:: build_exe.bat — DevPulse 后端打包为 Windows .exe
echo Building DevPulse Backend Executable...
