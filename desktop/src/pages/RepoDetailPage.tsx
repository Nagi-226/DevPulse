...
export function RepoDetailPage() {
  ...
  // Phase 4: 互动功能
  const likeInfo = useInteractionStore((s) => s.likeInfo);
  const likeLoading = useInteractionStore((s) => s.likeLoading);
  const toggleLike = useInteractionStore((s) => s.toggleLike);
  const fetchLikesCount = useInteractionStore((s) => s.fetchLikesCount);
  const resetInteraction = useInteractionStore((s) => s.reset);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      if (typeof navigator !== "undefined" && !navigator.onLine) {
        try {
          const cached = await getDetailCache(name);
          if (cached) {
  ...
  }, [owner, repoName, fetchRepo, fetchStarHistory, resetInteraction]);

  // 加载点赞数
  useEffect(() => {
    if (fullName && !offlineDataUsed) {
      fetchLikesCount(fullName);
    }
  }, [fullName, fetchLikesCount, offlineDataUsed]);
      ...
      <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-6">
        ...
        <div className="mt-4 flex items-center gap-3">
          <button
            ...
            disabled={likeLoading || offlineDataUsed}
            ...
            style={{ minHeight: 36 }}
          >
            <svg className="h-4 w-4" fill={likeInfo.liked ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
            </svg>
            点赞 {likeInfo.count > 0 ? likeInfo.count : ""}
          </button>
          <button
            onClick={handleToggleFavorite}
            className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition-all ${
          ...
          <div className="mt-4">
            <ResponsiveContainer width="100%" height={250}>
              ...
              >
                ...
                />
                <Line type="monotone" dataKey="stars" stroke="#818cf8" strokeWidth={2} dot={false} activeDot={{ r: 3, fill: "#818cf8", stroke: "#1e293b", strokeWidth: 2 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
      ...
      {repo.readme_summary && (
        <div className="mt-6 rounded-xl border border-slate-700 bg-slate-800/30 p-6">
          <h2 className="flex items-center gap-2 text-lg font-semibold text-white">
            ...
            {repo.confidence_score !== undefined && (
              <span className={`ml-2 rounded-full px-2 py-0.5 text-xs ${
                repo.confidence_score >= 0.7
                  ? "bg-emerald-500/15 text-emerald-400"
                  : repo.confidence_score >= 0.4
                  ? "bg-yellow-500/15 text-yellow-400"
                  : "bg-red-500/15 text-red-400"
              }`}>
                置信度 {Math.round(repo.confidence_score * 100)}%
              </span>
            )}
          </h2>

          <p className="mt-3 leading-relaxed text-slate-300">{repo.readme_summary}</p>
          ...
          {repo.tags && repo.tags.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {repo.tags.map((tag) => (
                <span key={tag} className="rounded-md bg-slate-700/50 px-2.5 py-1 text-xs text-slate-300">
                  #{tag}
                </span>
              ))}
            </div>
